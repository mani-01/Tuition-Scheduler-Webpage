...
confirmButton.onclick = function () {
    ...
    if (JSON.stringify(sanitisedInputs) !== JSON.stringify(allTrue)) { // a strange way of checking if arrays are equal but it works....
...
let userErrorDisplay = (sanitisedInputs) => {
    ...
    const formAndErrorTextHolderDiv = document.querySelector("#formAndErrorTextHolder");
    console.log(formAndErrorTextHolderDiv);


    let showWhatIsInvalidText = () => {
        let createNewStudentErrorTextToAppend = "";
        let fieldNamesArray = ["First Name", "Last Name", "Age", "Parent Name", "Contact Number"];
        let n = 0
        for (let i=0; i<5; i++) {
	...
	createNewStudentErrorText.textContent = "Error with fields: " + showWhatIsInvalidText() + ".";


    // checking if there is already an error box that has been put there and removing it
    if (document.contains(document.querySelector("#createNewStudentErrorText"))) {
